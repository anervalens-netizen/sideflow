...
class FloatingPanelService : Service() {
    ...
    companion object {
        ...
        const val ACTION_OPEN = "eu.astancu.sideflow.OPEN"
        const val ACTION_OPEN_HUB = "eu.astancu.sideflow.OPEN_HUB"
        const val ACTION_REFRESH = "eu.astancu.sideflow.REFRESH"
        const val ACTION_CLOSE_PANEL = "eu.astancu.sideflow.CLOSE_PANEL"
        const val ACTION_SHOW_TEMP = "eu.astancu.sideflow.SHOW_TEMP"
        const val ACTION_TOGGLE = "eu.astancu.sideflow.TOGGLE"
        const val ACTION_SCREENSHOT = "eu.astancu.sideflow.SCREENSHOT"
        const val ACTION_UPDATE_IMMERSIVE = "eu.astancu.sideflow.UPDATE_IMMERSIVE"
        const val ACTION_TOGGLE_FLASHLIGHT = "eu.astancu.sideflow.TOGGLE_FLASHLIGHT"
    ...
    private fun initSidePanel() {
        sidePanelView = SidePanelView(this).apply {
            ...
            onToolClick = { toolId ->
                when (toolId) {
                    "sideflow.tool.screenshot" -> triggerScreenshot()
                    "sideflow.tool.volume_up" -> adjustVolume(android.media.AudioManager.ADJUST_RAISE)
            ...
            val apps = if (currentFolderId != null) {
                when (currentFolderId) {
                    "sideflow.folder.tools" -> {
                        ...
                        tools.add(AppInfo("sideflow.tool.screenshot", "Screenshot", type = AppInfo.Type.TOOL))
                        
                        // Add Volume tools
                        tools.add(AppInfo("sideflow.tool.volume_up", "Volume +", type = AppInfo.Type.TOOL))
                        tools.add(AppInfo("sideflow.tool.volume_down", "Volume -", type = AppInfo.Type.TOOL))
                        
                        // Add Brightness tools
                        tools.add(AppInfo("sideflow.tool.brightness_up", "Brightness +", type = AppInfo.Type.TOOL))
                        tools.add(AppInfo("sideflow.tool.brightness_down", "Brightness -", type = AppInfo.Type.TOOL))
                        
                        // Always include power menu in the folder if the folder is active
                        tools.add(AppInfo("sideflow.shortcut.reboot", "Power Menu", type = AppInfo.Type.SHORTCUT))
